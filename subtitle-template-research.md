...
## 2026-07-15 用户样片增补：第 11 套

| ID | 通用名称 | 样片证据 | 与首批模板的差异 | 时间轴 | 状态 |
| --- | --- | --- | --- | --- | --- |
| `rolling-focus-subtitle` | 滚动聚焦大字字幕 | 用户提供 `7月15日.mp4`；1920×1080、30fps、35.922 秒 | 纯黑横屏、左侧约 5% 起始位置、3–5 行上下文、当前句白色粗体与 ▶、非当前句灰白；无描边/阴影/彩色强调 | 校正句级时间；有逐词时间时按停顿拆成 4–10 字语义组 | 已接入 |

- 样片只用于测量通用字幕行为，不复制视频、字体、代码或品牌资产。
- 正式实现使用独立 `rolling-focus-left` renderer mode，不覆盖首批 `rolling-focus`。
- 默认切换 220ms，可在 180–260ms 内随动画速度调整；提前量 90ms，长停顿 1200ms 后重置历史队列。
