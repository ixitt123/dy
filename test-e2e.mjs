...
test("TTS final-audio transcript alignment", async () => {
  ...
  if (!page.includes('id="ttsAlignmentEditor"')
    ...
    || !ttsServiceSource.includes("buildScriptLockedAlignment")
    || !ttsServiceSource.includes("automatic_singing_audio_lyrics")
    || !ttsServiceSource.includes("recognition_match_ratio")
...
test("Kinetic text production line", async () => {
  ...
  if (!effectsResponse.ok || effects.effects?.length !== 2
    ...
    || !moduleSource.includes('$("#kineticPreviewCanvas").addEventListener("pointerdown", beginOverlayDrag)')
    || !moduleSource.includes('$("#kineticPreviewCanvas").addEventListener("pointermove", moveOverlayDrag)')
    || !moduleSource.includes("bottomSubtitlePosition")
    || !moduleSource.includes("suppressCanvasClick")
    || !kineticServiceSource.includes("item.position?.x")
    || !kineticServiceSource.includes("normalized.bottomSubtitlePosition?.x")
