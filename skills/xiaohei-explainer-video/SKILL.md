...
- 风格：纯白画布、黑色手绘结构线、少量红/橙/蓝手写批注、原创可爱 3D 动画潮玩主角。
- 输出目录：`docs/<slug>-xiaohei-explainer/`。
- 本地渲染输出：`video-products/<slug>-xiaohei-explainer.mp4`，不要提交 GitHub。

## 角色系统：Xiaohei 2.0

旧“小黑”不能只是黑豆线稿，也不能是丑陋的黑色小球。默认升级为一个原创可爱 3D 动画潮玩主角：

- 黑色或深炭色主体，大头小身体，圆润头身比例，软胶 + 短绒 + 收藏玩偶质感。
- 有标志性的圆润三角耳或柔软小角轮廓，让角色第一眼可识别，不能只是一个黑豆。
- 大而清澈的玻璃高光眼，小鼻子、小嘴和一颗不夸张的小尖牙，表情认真、好奇、略带淘气。
- 小短手、小短腿，姿态要像动画电影角色一样有表演：拉线、搬运、推门、调机器、擦掉多余节点、托住锚点。
- 整体必须可爱、精致、有高级 3D 动画角色质感；禁止丑脸、恐怖脸、廉价盲盒感或低模玩具感。
- 造型要有识别度，但必须原创，不能复刻任何现有品牌角色或特定动画工作室风格。
- 不在最终 prompt 里写“Pixar”“Labubu”等品牌或 IP 名称；把用户的审美意图翻译成“original cute cinematic 3D designer-toy character, soft vinyl and plush texture, rounded ears, glossy eyes, tiny tooth, playful but original silhouette”。
...
- 保留：一张图/一个镜头只讲一个核心结构。
- 修改：主角不是丑线稿小黑，而是原创可爱 3D 动画潮玩主角，必须有圆润耳朵/小角、大眼高光、小鼻嘴、小尖牙和软胶短绒质感。
...
如果角色用 CSS 绘制，优先做成圆润、可爱的 3D 感：圆润耳朵/小角、玻璃眼高光、脸颊光、小鼻嘴、小尖牙、软阴影、柔和边缘。若要生成 bitmap 角色或配图，保存到本地资产目录，但不要提交大体积素材。
