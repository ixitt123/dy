  ...
  return async function handleIanXiaoheiRoutes(req, res, url) {
    ...
    if (req.method === "POST" && route === "export-external-prompts") {
      try {
        ...
        fs.mkdirSync(batchDir, { recursive: true });
        savePlanFiles(batchDir, plan);
        const queuePath = exportChatGptPromptQueue(batchDir, plan);
        openFile(queuePath);
        sendJson(res, 200, {
          ok: true,
          batchId,
...
function exportChatGptPromptQueue(batchDir, plan) {
  const promptDir = path.join(batchDir, "chatgpt-image-queue");
  fs.rmSync(promptDir, { recursive: true, force: true });
  fs.mkdirSync(promptDir, { recursive: true });
  const aspectRatio = normalizeAspectRatio(plan.aspectRatio);
  const prompts = [];
  for (const shot of plan.shots || []) {
    const index = clamp(Number(shot.index) || prompts.length + 1, 1, 99);
    const prompt = externalSingleImagePrompt(shot, { plan, aspectRatio });
    prompts.push({ index, topic: shot.topic || "", sourceText: shot.sourceText || "", prompt });
  }
  const queuePath = path.join(promptDir, "chatgpt-image-queue.html");
  fs.writeFileSync(queuePath, chatGptQueueHtml(prompts, { aspectRatio }), "utf8");
  fs.writeFileSync(path.join(promptDir, "manifest.json"), JSON.stringify({
    createdAt: new Date().toISOString(),
    batchId: plan.batchId || "",
    aspectRatio,
    count: prompts.length,
    usage: "ChatGPT 网页不会把多个附件当成多个生图任务。请打开 chatgpt-image-queue.html，用队列页一次复制一条 prompt 到 ChatGPT 网页生成。",
    prompts: prompts.map(({ index, topic, sourceText }) => ({ index, topic, sourceText })),
  }, null, 2), "utf8");
  fs.writeFileSync(path.join(promptDir, "README.txt"), [
    "ChatGPT 网页生图队列",
    "",
    "用法：",
    "1. 打开 chatgpt-image-queue.html。",
    "2. 点“复制当前提示词”，粘贴到 ChatGPT 网页并生成图片。",
    "3. 保存图片后回队列页点“下一张”，再复制下一条。",
    "4. 不要一次上传多个 txt 或一次粘贴多条 prompt，ChatGPT 网页会把它们合并成一张图。",
    "5. 本队列不调用本软件图片 API。",
  ].join("\n"), "utf8");
  return queuePath;
}

function chatGptQueueHtml(prompts = [], { aspectRatio = "16:9" } = {}) {
...
<head>
  ...
  <title>ChatGPT 网页生图队列</title>
  <style>
    * { box-sizing: border-box; }
    body { margin: 0; font-family: "Microsoft YaHei", Arial, sans-serif; background: #0b0f17; color: #eef4ff; }
    main { max-width: 980px; margin: 0 auto; padding: 24px; }
    header { display: flex; align-items: center; justify-content: space-between; gap: 16px; margin-bottom: 16px; }
    h1 { margin: 0; font-size: 24px; }
    .badge { color: #a8b3c7; font-size: 14px; }
    .panel { border: 1px solid #253044; background: #111722; border-radius: 8px; padding: 16px; }
    .toolbar { display: flex; flex-wrap: wrap; gap: 10px; margin: 14px 0; }
    button, a.button { border: 1px solid #394761; background: #172033; color: #fff; border-radius: 7px; padding: 10px 14px; font-weight: 700; cursor: pointer; text-decoration: none; }
    button.primary { background: #7057ff; border-color: #7057ff; }
    button:disabled { opacity: .45; cursor: not-allowed; }
    textarea { width: 100%; min-height: 520px; resize: vertical; border: 1px solid #2d3950; background: #070b12; color: #eaf1ff; border-radius: 8px; padding: 14px; line-height: 1.55; font-size: 15px; }
    .meta { color: #a8b3c7; margin: 8px 0 0; }
    .warn { color: #ffd166; }
  </style>
</head>
<body>
  <main>
    <header>
      <div>
        <h1>ChatGPT 网页生图队列</h1>
        <div class="badge">共 <span id="total">0</span> 张 · ${escapeHtml(aspectRatio)} · 每次只复制当前 1 条</div>
      </div>
      <a class="button" href="https://chatgpt.com/" target="_blank" rel="noopener noreferrer">打开 ChatGPT</a>
    </header>
    <section class="panel">
      <strong id="title">等待载入</strong>
      <p class="meta warn">不要把多条 prompt 一次性粘贴到 ChatGPT；ChatGPT 网页会合并成一张图。</p>
      <div class="toolbar">
        <button id="prev" type="button">上一张</button>
        <button id="copy" class="primary" type="button">复制当前提示词</button>
        <button id="next" type="button">下一张</button>
        <button id="mark" type="button">标记已完成</button>
      </div>
  ...
  <script>
    ...
    let index = Number(localStorage.getItem("chatgpt-image-queue-index") || 0);
    const done = new Set(JSON.parse(localStorage.getItem("chatgpt-image-queue-done") || "[]"));
    ...
    function save() {
      localStorage.setItem("chatgpt-image-queue-index", String(index));
      localStorage.setItem("chatgpt-image-queue-done", JSON.stringify([...done]));
    }
    function render() {
      if (!prompts.length) {
        title.textContent = "没有提示词";
        promptBox.value = "";
        return;
      }
      index = Math.max(0, Math.min(index, prompts.length - 1));
      const item = prompts[index];
      total.textContent = String(prompts.length);
      title.textContent = \`第 \${index + 1} / \${prompts.length} 张：\${item.topic || "未命名"}\${done.has(item.index) ? "（已完成）" : ""}\`;
      promptBox.value = item.prompt;
      prev.disabled = index <= 0;
      next.disabled = index >= prompts.length - 1;
      status.textContent = \`当前只复制第 \${index + 1} 张。生成并保存图片后，再点下一张。\`;
      save();
    }
    async function copyPrompt() {
      ...
      try {
        ...
        status.textContent = \`已复制第 \${index + 1} 张提示词。现在去 ChatGPT 网页粘贴生成。\`;
      } catch {
        document.execCommand("copy");
        status.textContent = \`已复制第 \${index + 1} 张提示词。\`;
      }
    }
    ...
    mark.addEventListener("click", () => {
      if (prompts[index]) done.add(prompts[index].index);
      if (index < prompts.length - 1) index += 1;
      render();
    });
    render();
  </script>
...
function openFile(filePath) {
  const resolved = path.resolve(String(filePath || ""));
  if (!fs.existsSync(resolved) || !fs.statSync(resolved).isFile()) throw new Error("要打开的文件不存在。");
  if (process.platform === "win32") {
    spawn("cmd.exe", ["/c", "start", "", resolved], { detached: true, stdio: "ignore", windowsHide: true }).unref();
    return;
  }
  const command = process.platform === "darwin" ? "open" : "xdg-open";
  spawn(command, [resolved], { detached: true, stdio: "ignore" }).unref();
}
