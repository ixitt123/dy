...
<head>
  ...
  <style>
    *{box-sizing:border-box}
    ...
    #root{position:relative;width:${width}px;height:${height}px;overflow:hidden;background:#17170c}
    #stage{position:absolute;left:${fit.left}px;top:${fit.top}px;width:${baseWidth}px;height:${baseHeight}px;overflow:hidden;transform:scale(${fit.scale});transform-origin:top left;background:#252512;font-size:0;line-height:0}
...
function warmGrainFiles(model, { aspect = ASPECT_RATIO_PRESETS["9:16"] } = {}) {
  const width = aspect.width;
  const height = aspect.height;
  const baseWidth = 1920;
  const baseHeight = 1080;
