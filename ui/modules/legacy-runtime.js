...
document.querySelector("#sendTtsToCs1")?.addEventListener("click", async () => {
  if (!activeTtsRailJob?.id || activeTtsRailJob.status !== "completed") {
  ...
  try {
    ...
    window.videoFactorySendToCs1?.(job, window.videoProjects?.current?.());
    ttsStatus.textContent = "已确认当前语音并发送到 CS1 文字视频。";
  } catch (error) {
    ttsStatus.textContent = error instanceof Error ? error.message : String(error);
  }
});
...
document.querySelector("#sendTtsToMoneyPrinterTurbo")?.addEventListener("click", async () => {
  if (!activeTtsRailJob?.id || activeTtsRailJob.status !== "completed") {
    ttsStatus.textContent = "请先生成、试听并确认一条语音。";
    return;
  }
  try {
    ...
    window.videoFactorySendToMoneyPrinterTurbo?.(job, window.videoProjects?.current?.());
    ttsStatus.textContent = "已确认当前语音并发送到 MoneyPrinterTurbo。";
  } catch (error) {
    ttsStatus.textContent = error instanceof Error ? error.message : String(error);
  }
});
